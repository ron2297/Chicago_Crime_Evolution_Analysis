...
# 
# Assualt_to_Theft_Dist = nncross(crimePPP_Assault,crimePPP_Theft)
# #hist(Battery_to_Damage_Dist$dist,main="")
# 
# 
# 
# # DONT TOUCH
...
X = crimePPP_Assault
#for (i in 2: length(Y)){ X = superimpose(X,Y[[i]])}
sapply(seq_along(Y),function(X){ X = superimpose(X,Y[[i]])})
...
# Kest.
# #k12 = Kcross.inhom(X,"Assualt","Theft",assualt_density,theft_density,r =seq (min(Assualt_to_Theft_Dist$dist) ,summary(Assualt_to_Theft_Dist$dist)[[5]],length.out = 2) , main = "", ribwid = 0.06, ribsep = 0.06)
# g122 = pcfcross.inhom(X,r=NULL,breaks=NULL, correction = "best")
# plot(g122)
#Ginhom(X,sigma=10)
soethin = pcfcross.inhom(X, "Assualt","Theft",assualt_density,theft_density,legendpos="bottom")
